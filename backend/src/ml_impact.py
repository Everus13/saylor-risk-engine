...
class QuantileLoss(nn.Module):
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        for i, q in enumerate(self.quantiles):
            ...
            loss = loss + torch.max(q * error, (q - 1) * error).mean()
        return loss

# --- PyTorch Quantile Regression MLP Model ---
class QuantileMLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dims: Optional[List[int]] = None, output_dim: int = 3) -> None:
        # hidden_dims is kept for backward compatibility in signature but ignored for the DeepBNGelu arch
        super().__init__()
...
# --- Feature Engineering ---
def extract_features(order_book: Dict[str, Any], sell_size: float) -> np.ndarray:
    """
...
# --- Dataset Generator ---
def generate_ml_dataset(n_snapshots: int = 200, levels: int = 50) -> Tuple[np.ndarray, np.ndarray]:
    """
...
class PriceImpactMLPipeline:
    def __init__(self) -> None:
        self.best_lgb_models: Dict[float, lgb.LGBMRegressor] = {}
        self.best_pytorch_model: Optional[QuantileMLP] = None
    ...
    def train_lightgbm_quantiles(
        self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, quantiles: List[float] = [0.1, 0.5, 0.9]
    ) -> None:
        """
        ...
        """
        logging.info("Training LightGBM Quantile Models...")
        ...
        for q in quantiles:
            ...
            logging.info(f"Quantile {q:.1f}: Best Params = {best_params}, Val Loss = {best_loss:.6f}")
        ...
        logging.info(f"Saved LightGBM models to {LIGHTGBM_MODEL_PATH}")

    def train_pytorch_quantiles(
        self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, quantiles: List[float] = [0.1, 0.5, 0.9]
    ) -> None:
        """
        Train a PyTorch Multi-Quantile MLP.
        """
        logging.info("Training PyTorch Quantile Neural Network...")
        ...
        for lr in learning_rates:
            for hidden in hidden_options:
                ...
                optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=float(PYTORCH_CONFIG.get("weight_decay", 1e-4)))
                
                train_loader = DataLoader(train_dataset, batch_size=int(PYTORCH_CONFIG["batch_size"]), shuffle=True)
        ...
        logging.info(f"PyTorch Grid Search complete. Best LR = {best_lr}, Val Loss = {best_val_loss:.6f}")
        ...
        optimizer = optim.Adam(self.best_pytorch_model.parameters(), lr=best_lr, weight_decay=float(PYTORCH_CONFIG.get("weight_decay", 1e-4)))
        criterion = QuantileLoss(quantiles=quantiles)
        train_loader = DataLoader(train_dataset, batch_size=int(PYTORCH_CONFIG["batch_size"]), shuffle=True)
        
        for epoch in range(int(PYTORCH_CONFIG["epochs"])):
